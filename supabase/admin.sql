...
-- =============================================================================
-- Run this AFTER schema.sql (Supabase → SQL Editor). Idempotent.
--
-- WHAT THIS ADDS:
...
--   * Admin policies granting full (view + edit) access to every data table.
--
...
-- ---------------------------------------------------------------------------
drop policy if exists "admin - all workout_plans" on public.workout_plans;
create policy "admin - all workout_plans" on public.workout_plans
  for all using (public.is_admin()) with check (public.is_admin());

drop policy if exists "admin - all user_preferences" on public.user_preferences;
create policy "admin - all user_preferences" on public.user_preferences
  for all using (public.is_admin()) with check (public.is_admin());
