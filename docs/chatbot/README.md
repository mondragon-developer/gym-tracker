...
| `08-plan-format-en.md` | English | The GYMPLAN v1 block the bot must end every plan answer with, so the app's Import plan screen can build the week; full bilingual exercise and group tables. Generated by `npm run build:plan-doc`, do not edit by hand |
| `08-formato-plan-es.md` | Spanish | Same content in Spanish |
